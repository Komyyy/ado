...
lemma pbw_span : span R ((prod ∘ map (ι R ∘ B)) '' {l : List m | SortedLE l}) = ⊤ := by
    ...
    case neg =>
      ...
      · replace hx := mul_mem_mul (mem_span_singleton_self (ι R (B b))) hx
        ...
        · refine mem_of_le_of_mem ?_ hx
          ...
          simp [hl']
        · conv_rhs =>
            equals ι R (Finsupp.linearCombination R B (B.repr ⁅B b, B a⁆)) *
              prod (map (ι R ∘ B) l) => simp
          conv_rhs => arg 1; apply Finsupp.apply_linearCombination _ (ι R).toLinearMap
          simp_rw [LieHom.coe_toLinearMap, Finsupp.mem_span_image_iff_linearCombination]
          conv_rhs =>
            tactic => simp_rw [Finsupp.linearCombination_apply, Finsupp.sum_mul,
              ← Finsupp.linearCombination_apply]
          have hli : Injective ((· :: l) : m → List m) := fun _ _ _ ↦ by simp_all
          exists Finsupp.embDomain ⟨(· :: l), hli⟩ (B.repr ⁅B b, B a⁆)
          simp [comp_def, Finsupp.mem_supported, - ι_apply]
